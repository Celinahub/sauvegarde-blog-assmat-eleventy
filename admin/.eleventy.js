...
eleventyConfig.addCollection("actualites", function(collectionApi) {
  return collectionApi.getAll().filter(function(item) {
    return item.data.tags && item.data.tags.includes("actualites");
  });
});
...
// Collection pour rpe
eleventyConfig.addCollection("rpe", function(collectionApi) {
  return collectionApi.getAll().filter(function(item) {
    return item.data.tags && item.data.tags.includes("rpe");
  });
});
...
// Ajouter la pagination à la catégorie collectivite-rpe
eleventyConfig.addCollection("collectivite-rpe", function(collectionApi) {
  return collectionApi.getFilteredByGlob("posts/*.md")
    .filter(post => post.data.category === "LA COLLECTIVITE GRÂCE AU RPE")
    .sort((a, b) => b.date - a.date);
});
